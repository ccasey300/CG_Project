		...
		std::vector<glm::mat4> &nodeTransforms) 
	{
		...
		for (const auto &channel : anim.channels) {
			...
			if (channel.target_path == "translation") {
				glm::vec3 translation0, translation1;
				memcpy(&translation0, outputPtr + keyframeIndex * 3 * sizeof(float), 3 * sizeof(float));
				//get translation for next key frame
				memcpy(&translation1, outputPtr + nextKeyframeIndex * 3 * sizeof(float), 3 * sizeof(float));

				//changed, incorporate interpolation
				glm::vec3 translation = glm::mix(translation0, translation1, factor);
				//nodeTransforms[targetNodeIndex] = glm::translate(nodeTransforms[targetNodeIndex], translation);
				nodeTransforms[targetNodeIndex] = glm::translate(glm::mat4(1.0f), translation);

				//debugging
				//std::cout << "Translation Keyframes: " << "t0: " << glm::to_string(translation0) << ", t1: " << glm::to_string(translation1) << std::endl;
				...
				//debugging
				//std::cout << "Rotation Keyframes: " << "r0: " << glm::to_string(rotation0) << ", r1: " << glm::to_string(rotation1) << std::endl;
				...
				//debugging
				//std::cout << "Scale Keyframes: " << "s0: " << glm::to_string(scale0) << ", s1: " << glm::to_string(scale1) << std::endl;
