...
struct MyBot {
		...
		std::vector<glm::mat4> &localTransforms)
	{
		...
		// TODO: your code here
		// ---------------------------------------
		//from previous skeleton code
		// Retrieve the node
		const tinygltf::Node& node = model.nodes[nodeIndex];

		// Compute the local transform for the current node
		glm::mat4 transform = getNodeTransform(node);

		// Store the local transform in the corresponding index
	...
	// ----------------------------------------
	{
		// Combine parent's global transform with the node's local transform
		glm::mat4 globalTransform = parentTransform * localTransforms[nodeIndex];
		globalJointTransforms[nodeIndex] = globalTransform;

		// Traverse child nodes recursively
		const tinygltf::Node &node = model.nodes[nodeIndex];
		for (int childIndex : node.children) {
			computeGlobalNodeTransform(model, localTransforms, childIndex, globalTransform, globalJointTransforms);
		}

		//debugging:
/*
		std::cout << "Local Transforms:" << std::endl;
		for (size_t i = 0; i < localTransforms.size(); ++i) {
			std::cout << "Node " << i << ": " << glm::to_string(localTransforms[i]) << std::endl;
		}

		std::cout << "Global Transforms:" << std::endl;
		for (size_t i = 0; i < globalJointTransforms.size(); ++i) {
			std::cout << "Node " << i << ": " << glm::to_string(globalJointTransforms[i]) << std::endl;
		}
*/
	}
	...
	*/
	...
	std::vector<SkinObject> prepareSkinning(const tinygltf::Model &model) {
		...
		for (size_t i = 0; i < model.skins.size(); i++) {
			...
			// Compute joint matrices: JointMatrix = GlobalTransform * InverseBindMatrix
...
/*
	...
	void updateSkinning(const std::vector<glm::mat4> &nodeTransforms) {
		...
		for (size_t i = 0; i < skinObjects.size(); i++) {
			...
			for (size_t j = 0; j < skin.joints.size(); j++) {
				...
				skinObject.jointMatrices[j] = skinObject.globalJointTransforms[j] * skinObject.inverseBindMatrices[j];
				// Debugging output, shows correct output
				std::cout << "Joint " << j << " matrix:\n" << glm::to_string(skinObject.jointMatrices[j]) << std::endl;
			}
		}
	}
	...
	void update(float time) {
		...
		if (model.animations.size() > 0) {
			...
			SkinObject &skinObject = skinObjects[0]; // Reference the first skin
			...
			computeGlobalNodeTransform(model, nodeTransforms, skin.joints[0], parentTransform, skinObject.globalJointTransforms);
	...
	void render(glm::mat4 cameraMatrix) {
		...
		// -----------------------------------------------------------------
		const SkinObject &skinObject = skinObjects[0]; // Use the first skin object

		if (skinObject.jointMatrices.size() > 0) {
			// Debugging: Output the number of joint matrices being passed
			std::cout << "Number of joint matrices: " << skinObject.jointMatrices.size() << std::endl;

			for (size_t i = 0; i < skinObject.jointMatrices.size(); ++i) {
				std::string uniformName = "jointMatrices[" + std::to_string(i) + "]";
				GLint location = glGetUniformLocation(programID, uniformName.c_str());

				// Debugging: Check if the uniform location was retrieved correctly
				if (location == -1) {
					std::cerr << "Error: Could not find uniform " << uniformName << " in shader program!" << std::endl;
				} else {
					// Debugging: Output the joint matrix for this joint
					//std::cout << "Setting " << uniformName << " to matrix: " << std::endl;
					//for (int row = 0; row < 4; ++row) {
					//	std::cout << glm::to_string(skinObject.jointMatrices[i][row]) << std::endl;
					//}

					// Pass the joint matrix to the shader
					//
					//glUniformMatrix4fv(location, skinObject.jointMatrices.size(), GL_FALSE, &skinObject.jointMatrices[i][0][0]);
					//second attemt to pass:

					glUniformMatrix4fv(location, skinObject.jointMatrices.size(), GL_FALSE,
		...
		/*
		for (size_t i = 0; i < skinObjects[0].jointMatrices.size(); ++i) {
			std::string uniformName = "jointMatrices[" + std::to_string(i) + "]";
			GLint location = glGetUniformLocation(programID, uniformName.c_str());
			glUniformMatrix4fv(location, 1, GL_FALSE, &skinObjects[0].jointMatrices[i][0][0]);
		}*/
		//above produces the weird "worm"
		//const SkinObject &skinObject = skinObjects[0]; // Use the first skin
		...
		/*
				//debugging, check that whats sent to shader makes sense:
				for (size_t i = 0; i < skinObjects[0].jointMatrices.size(); ++i) {
					std::cout << "Joint " << i << " matrix:\n" << glm::to_string(skinObjects[0].jointMatrices[i]) << "\n";
		*/
	}
...
layout(location = 4) in vec4 jointWeights;  // Joint weights, specifcty how 3D vertices should deform
...
uniform mat4 MVP;
//joint matrices
uniform mat4 jointMatrices[25];      // Joint matrices (20 joints, make space for more
